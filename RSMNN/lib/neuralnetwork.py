...
def monotone_ffnn(input_data, 
    ...
    if isinstance(width, list):
        if isinstance(width[0], list):
            ...
            layer_widths = [[w] * len(activations) for w in width]
    else:
...
#            self.Yhat_tf = self.Y_hats[0]
        ...
        self.learning_rate = tf.placeholder(shape=[], dtype=self.dtype)
        self.optimizer_instance = self.optimizer(learning_rate=self.learning_rate)
        self.train_op = self.optimizer_instance.minimize(loss=self.loss, var_list=self.vars)
    ...
    def apply_np(self, X, session, ):
        return session.run(self.Y_tf, feed_dict={self.X_tf: X})
